...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	...
	mysqli_query($link,$aa);
		...
		$ts1="insert into duebill(mrno,billno,bdate,pname,mobile,pamount,user,bill_num,time) values('$mrno','$bno','$bdate','$pname','$mobile','$damount','$user','$cnt','$time')";
		$crud->execute($ts1);
		...
		if($result){
			...
			print "self.location='print_dllabbillk.php?id=$cnt';";
			print "</script>";
		}
	}	
