...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	...
	if($rupees>=1){
		$p="insert into adv_collection(PAT_NO, ADV_DATE, ADV_AMT, PAYMENT_MODE, CURRENTDATE, AUTH_BY,bill_num,admit_date,pname,age,gender,mobile,rwords,mrno)
 values('$patno','$bdate','$rupees','$paymenttype',now(),'$user','$cnt','$admitdate','$pname','$age','$gender','$mobile','$adv_words','$mrno')";
$sql1 = $crud->execute($p);
} 
