...
if(isset($_POST['Submit'])) {	
	...
	$paymenttype=$_POST['paymenttype'];
	 $patientcategory=$_POST['patient-cateory'];
	$now_pay=$_POST['now_pay'];
	$now_bal=$_POST['now_bal'];
	$user=$_POST['user'];

	$sql = mysqli_query($link,"select max(id+0) from final_bill_fin1 ");
...
if($sql)
{
 ...
 $lrno=$lrno+1;
   ...
   $a="INSERT INTO final_bill_fin1(bno,mrno,patno,pname,fname,age,sex,mobile,
		...
		lab_bal_amnt,tot_pharma_amnt,pharma_paid,pharma_bal,total,paid,due,concession,namount,tot_proc_amnt,proc_paid_amnt,proc_bal_amnt,paymenttype,new_paid,new_bal,bdate,btime,patientcategory,user)VALUES('$bno',
		...
		'$pharma_bal1','$total1','$paid1','$due1','$concession1','$namount1','$tot_proc_amnt','$proc_paid_amnt','$proc_bal_amnt','$paymenttype','$now_pay','$now_bal','$bdate','$time','$patientcategory','$user')";
...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	$aa="insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)
	 values('Final Bill Insurance','$now_pay','$cnt','$mrno','$user','$paymenttype','$ndate','Final Bill Insurance')";
...
for($m=0;$m <= $count;$m++)
{
 ...
 $remarks=$_POST['remarks'][$m];
$days=$_POST['days'][$m];
$charge=$_POST['charge'][$m];
$amount=$_POST['amount'][$m];
if($description!=''){
		  $a="insert into final_bill_genral1(desc1, days, charge, amnt, id1,remarks) 
values('$description','$days','$charge','$amount','$lrno','$remarks')";
...
if($sql)
{
 ...
 $lrno=$lrno+1;
   ...
   $a="update   final_bill_fin1 set bno='$bno',mrno='$mrno',patno='$patno',pname='$patname',fname='$fname',
   age='$age',sex='$sex',mobile='$mobile',
		...
		namount='$namount1',tot_proc_amnt='$tot_proc_amnt',proc_paid_amnt='$proc_paid_amnt',proc_bal_amnt='$proc_bal_amnt',bdate='$bdate',btime='$time',new_paid='$now_pay',new_bal='$now_bal',patientcategory='$patientcategory'
		 where id='$id'";
