...
$id=$crud->my_simple_crypt($_REQUEST['id'],'d');
			...
			$tp="select *  from mfinalbill where billno='$id'";
			$rst = $crud->getdata($tp);
...
$mrno = $crud->escape_string($_POST['mrno']);
	...
	$id=$crud->my_simple_crypt($_POST['id'],'d');
		  ...
		  $ts="update mfinalbill set mrno='$mrno',billno='$bno',bdate='$bdate',pname='$pname',age='$age',gender='$gender',dname='$dname',mobile='$mobile',ptype='$ptype',time='$time',tamount='$tamount',discount='$discount',namount='$namount',pamount='$pamount',bamount='$balamount',remarks='$remarks',user='$user',paymenttype='$paymenttype' where id='$id'";
		...
		$amount= $_POST['amount'][$t];
		 $id1= $_POST['id1'][$t];
			if($tname!='' and $id1!=''){
				  $ts1="update mfinalbill1 set testname='$tname',amount='$amount' where id1='$id1' and mrno='$mrno'";
	...
	//exit;
		if($result){
			...
			print "self.location='mfinalbilllist.php';";
			print "</script>";
		}
	}	
