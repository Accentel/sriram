...
function card_pop(b,a,str){
	...
	function card_pop1(b,a,str){
	if(a==1){
	window.open('../modal/reg_card_print1.php?opno='+b+'&a='+a+'','mywindow','width=700,height=500,toolbar=no,menubar=no,scrollbars=yes');
	}
	if(a==2){
	window.open('../modal/reg_card_print.php?opno='+b+'&a='+a+'&age='+str+'','mywindow','width=700,height=500,toolbar=no,menubar=no,scrollbars=yes');
...
<td>
					                                    ...
					                                    <div class="table-scrollable">
															...
															if($mobile!=''){
																	...
																	$sq=mysqli_query($link,"select * from patientregister  where phoneno='$mobile' and pcancel='' and status!='Delete' order by reg_id desc");
															}
															
															else if($mr_num!=''){
																$sq=mysqli_query($link,"select * from patientregister  where registerno='$mr_num' and pcancel='' and status!='Delete' order by reg_id desc");
															}
																
																else if($village!=''){
																$sq=mysqli_query($link,"select * from patientregister  where address='$village' and pcancel='' and status!='Delete' order by reg_id desc");
															}else {
																
															$sq=mysqli_query($link,"select * from patientregister  where  pcancel='' and status!='Delete'  order by reg_id desc");
															}
			  ...
			  $t = $r['tokenno'];
                             ?>    
																	...
																	<td class="left"><?php echo $r['pat_type'];?></td>
...
$dname1="Dr.RAJESH SONI";
	...
	$dname1=$dnamek;
															...
															<?php if($ses!='admin'){ ?>
																...
																<A onclick="card_pop('<?php echo $o ?>',1,'<?php echo $t ?>')">
																	...
																	<img src="../img/print.png">
																		<!--<button class="btn btn-danger btn-xs">	
																			...
																			<a href="due_patientregister.php?id=<?php echo $r['reg_id'] ?>"  >
																...
																<A onclick="card_pop1('<?php echo $o ?>',1,'<?php echo $t ?>')">
